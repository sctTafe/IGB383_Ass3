...
public partial class MechAIDecisions : MechAI {
    ...
    private ResourceRiskTeir _riskTierOfCurrentResroucePointAt;
    private bool Utility_IsCurrentlyOnAResorucePoint()
    {
        ...
        foreach (var rp in _ResourcePoints)
        {
            if(Vector3.Distance(rp.transform.position, this.transform.position) < 1)
            {
                ...
                _riskTierOfCurrentResroucePointAt = _RankedResourcePointsList.FirstOrDefault(tuple => tuple.Item1 == rp).Item2;
            }
    ...
    {
        return _riskTierOfCurrentResroucePointAt == ResourceRiskTeir.low;
    }
    ...
    private float _laser_BaseFireAngle = 30f;
    ...
    private float _cannon_BaseFireAngle = 27f;
